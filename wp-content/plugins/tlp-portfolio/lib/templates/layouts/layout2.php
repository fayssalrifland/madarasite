...
if(in_array( 'client_name', $items ) && $client_name){
    $exFeature .= '<li class="client-name"><label>'.__("Client Name :", 'tlp-portfolio').'</label>'.$client_name.'</li>';
}
if(in_array( 'completed_date', $items ) && $completed_date){
    $exFeature .= '<li class="completed-date"><label>'.__("Completed Date :", 'tlp-portfolio').'</label>'.$completed_date.'</li>';
}
if(in_array( 'project_url', $items ) && $project_url){
    $exFeature .= '<li class="project-url"><label>'.__("Project URL :", 'tlp-portfolio').'</label><a  href="' . esc_url($item_link) . '" target="_blank">'.esc_url($item_link).'</a></li>';
}
...
if ($img) {
    $imgWithHtml = sprintf('<div class="tlp-portfolio-thum tlp-item %s">
                %s
...
<div class="<?php echo esc_attr($grid) ?>">
    <div class="tlp-portfolio-item rt-row">
        ...
        <?php if( $project_title || $exFeature ) { ?>
            <?php $imgWithHtml .= sprintf('<div class="tlp-content tlp-content2 %s">
